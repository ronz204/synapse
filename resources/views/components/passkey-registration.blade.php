...
{{--
    Registers a new passkey for the signed-in user. Expects the surrounding
    Livewire component to expose loadPasskeys(), which it calls to refresh the
    list once the browser has stored the credential.
--}}
<div
    x-data="{
        supported: false,
    ...
    <template x-if="supported && showForm">
        <div class="space-y-4 rounded-lg border border-border-default bg-background-subtle p-4">
            <flux:input
            ...
            <p x-show="error" x-text="error" x-cloak class="text-[13px] text-status-danger"></p>
